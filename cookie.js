...
/**
 * Set default consent states for Google Consent Mode v2 and Microsoft UET
 * This covers all required parameters for Google Consent Mode v2 including:
 * - ad_storage, analytics_storage (basic)
 * - ad_user_data, ad_personalization (advanced)
 * - functionality_storage, personalization_storage, security_storage
 */
function setDefaultConsentStates() {
    // Set default Google consent (deny all except security)
    ...
    // Set default Microsoft UET consent if enabled
    setDefaultUetConsent();
}

/**
 * Set default Microsoft UET consent with validation
 * Includes validation to ensure UET tag exists before setting consent
 */
function setDefaultUetConsent() {
    if (!config.uetConfig.enabled) return;
    
    try {
        // Validate UET tag exists if we're going to set consent
        if (typeof window.uetq === 'undefined') {
            console.warn('Microsoft UET tag not detected on page');
            window.uetq = [];
            return;
        }
        ...
        console.error('Error setting Microsoft UET consent:', e);
    }
}
...
// Track new cookie categories detected
let newCookieCategoriesDetected = false;
...
function generateAnalyticsDashboard(language = 'en') {
    try {
        ...
        return '<p>Error loading analytics data. Please try again.</p>';
    }
}
...
function getAvailableLanguages() {
    try {
        ...
        return ['en', 'fr']; // Default to English and French if error occurs
    }
}
...
function initializeCookieConsent(detectedCookies, language) {
    try {
        ...
        // Setup periodic cookie scan to detect new cookie categories
        setInterval(() => {
            const newCookies = scanAndCategorizeCookies();
            const currentCategories = Object.keys(detectedCookies);
            const newCategories = Object.keys(newCookies).filter(cat => 
                !currentCategories.includes(cat) && newCookies[cat].length > 0
            );
            
            if (newCategories.length > 0) {
                newCookieCategoriesDetected = true;
                if (config.behavior.acceptOnContinue) {
                    // Show banner again if new cookie categories are detected
                    showCookieBanner();
                }
            }
...
function clearNonEssentialCookies() {
    try {
        const cookies = document.cookie.split(';');
        const essentialCookies = ['cookie_consent', 'preferred_language', 'dashboard_auth', 'first_visit_date', 'session_start_time'];
        
        cookies.forEach(cookie => {
            const [nameValue] = cookie.trim().split('=');
            const [name] = nameValue.split(';');
            
            // Check if the cookie is non-essential and not in our essential list
            if (!essentialCookies.includes(name) && !config.essentialCookies.includes(name)) {
                // Clear the cookie by setting expiration to past date for all possible paths and domains
                document.cookie = `${name}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;`;
                
                // Also try to clear with domain if we're on a subdomain
...
function clearCategoryCookies(category) {
    try {
        ...
        cookies.forEach(cookie => {
            ...
            for (const cookiePattern in cookieDatabase) {
                if (name.startsWith(cookiePattern) || name === cookiePattern) {
                    if (cookieDatabase[cookiePattern].category === category) {
...
function loadCookiesAccordingToConsent(consentData) {
    try {
        ...
        // If analytics is rejected, clear analytics cookies
        if (consentData.categories.analytics === false) {
            clearCategoryCookies('analytics');
        }
        
        // If advertising is rejected, clear advertising cookies
        if (consentData.categories.advertising === false) {
            clearCategoryCookies('advertising');
        }
        
        // If performance is rejected, clear performance cookies
        if (consentData.categories.performance === false) {
            clearCategoryCookies('performance');
        }
        
        // If uncategorized is rejected, clear uncategorized cookies
        if (consentData.categories.uncategorized === false) {
...
document.addEventListener('DOMContentLoaded', function() {
    try {
        ...
        if (config.analytics.trackPageViews) {
            window.dataLayer.push({
                'event': 'cookie_consent_page_view',
